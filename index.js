...
const NDependAnalyzerHash =
  "0261c493c1df2789c402cb85c3fb81877acd3e2943136f819862ac540a39501e";
fs = require("fs");
path = require("path");
...
function populateTrends(dir) {
  fs.readdirSync(dir).forEach((file) => {
    let fullPath = path.join(dir, file);
    if (fs.lstatSync(fullPath).isDirectory()) {
      if (fullPath.indexOf("TrendMetrics") > 0) populateTrends(fullPath);
    } else {
      if (fullPath.indexOf("TrendMetrics") > 0) trendFiles.push(fullPath);
    }
  });
}
...
function getNDependResult(ndependFolder) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    if (stat.isDirectory()) {
      if (path.basename(filename) != "Baseline") getNDependResult(filename);
...
async function checkIfNDependExists(
  owner,
  repo,
  runid,
  octokit,
  ...
  for (const artifactKey in artifacts.data.artifacts) {
    ...
    if (artifact.name == "ndepend" && !artifact.expired) {
      ...
      const baselineExtractedFolder = await tc.extractZip(
        NDependBaseline,
        baseLineDir
      );
      return true;
...
async function run() {
  try {
    ...
    const artifactNameBase = core.getInput("artifactname");
    ...
    if (license == "")
      core.setFailed(
        "The ndepend license is not specified, Please ensure that the license input is present in your workflow."
      );

    if (license != "" && license.indexOf("<NDepend") < 0)
      core.setFailed(
        "The ndepend license is not valid, Please check your license data."
      );

    //get ndepend and extract it
    const ndependToolURL = await tc.downloadTool(
      "https://www.codergears.com/protected/GitHubActionAnalyzer.zip"
    );
    ...
    const tooldata = fs.readFileSync(
    ...
    var NDependParser =
      _getTempDirectory() +
      "/NDepend/GitHubActionAnalyzer/GitHubActionAnalyzer.exe";
    const licenseFile =
      _getTempDirectory() +
      "/NDepend/GitHubActionAnalyzer/NDependGitHubActionProLicense.xml";
    const configFile =
      _getTempDirectory() +
      "/NDepend/GitHubActionAnalyzer/NDependConfig.ndproj";
    ...
    if (baseline != "") {
      if (isGitHubRunId(baseline)) {
        ...
        try {
          ...
          baselineFound = await checkIfNDependExists(
            owner,
            repo,
            run.data.id,
            octokit,
          ...
          if (error.status === 404) {
            ...
            core.warning(
              "No NDepend artifacts found for this run id :" + baseline
            );
          }
        }
      } else {
        ...
        runs = await octokit.request(
          "Get /repos/{owner}/{repo}/actions/workflows/{workflow_id}/runs?status=completed&per_page=100&branch={branch}",
          {
            owner,
            repo,
            workflow_id,
        ...
        for (const runkey in runs.data.workflow_runs) {
          ...
          if (run.repository.name == repo) {
            ...
            if (baselineFound) {
              core.info(
                "Baseline to compare with has the run number:" + run.run_number
              );
              break;
            }
          }
      ...
      if (baseline != "" && !baselineFound) {
        if (baseline.indexOf("recent") < 0 && isNaN(baseline))
          core.warning(
            "The baseline value " +
              baseline +
              " is not valid. Valid values are recent , branch_recent, specific run number"
          );
        else core.warning("No baseline to compare found for :" + baseline);
      }
    }
    var args = [
      "/sourceDirectory",
      ...
      repo,
      "/buildId",
      currentRunNumber + " Id " + currentRunID,
    ];

    var configfilePath = workspace + "/" + configPath;
    if (!fs.existsSync(configfilePath)) {
      core.warning(
        "The NDepend custom config file " +
          configPath +
          " is not found, a default config file will be used instead."
      );
    }
    ...
    if (configPath != "" && fs.existsSync(configfilePath)) {
      ...
      if (solutions.length == 1) {
        ...
        if (solution != "") {
          ...
          core.setFailed(
            "More than one VS solution is found in this repository, please specify which one you want to parse from the action inputs"
          );
    ...
    if (
      baselineFound &&
      ndependResultFile != "" &&
      fs.existsSync(ndependResultFile)
    ) {
    ...
    if (stopifQGfailed == "true") args.push("/stopBuild");
    var ret = 0;
    try {
      var isLinux = process.platform === "linux";
      if (isLinux) {
        var NDependLinuxParser =
          _getTempDirectory() +
          "/NDepend/GitHubActionAnalyzer/net6.0/GitHubActionAnalyzer.MultiOS.dll";
        args.unshift(NDependLinuxParser);
        ret = await exec.exec("dotnet", args);
      } else ret = await exec.exec(NDependParser, args);
    } catch (error) {}

    const artifactClient = new DefaultArtifactClient();
    const artifactName = artifactNameBase;
    const artifactNameTrend = artifactNameBase + "_trend";
    ...
    if (context.payload.pull_request != null) {
      ...
      if (fs.existsSync(NDependOut + "/comment.txt")) {
        ...
        message =
          message +
          '\nTo have more details about the analysis you can Download the detailled report here ("https://github.com/' +
          owner +
          "/" +
          repo +
          "/actions/runs/" +
    ...
    if (fs.existsSync(NDependOut + "/comment.txt")) {
      ...
      if (
        message.indexOf("at least one Quality Gate failed") > 0 &&
        stopifQGfailed == "true"
      )
        core.setFailed(
          "The NDepend action failed the build because at least one Quality Gate failed and stopIfQGFailed is set to true  in the action options."
        );
    }
  } catch (error) {
